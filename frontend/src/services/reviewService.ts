...
export const reviewService = {
   ...
   */
  getReviewsByPost: async (postId: number | string, page: number = 0, size: number = 10): Promise<ReviewResponse[]> => {
    const response = await api.get<ApiResponse<ReviewResponse[]>>(`/v1/reviews/post/${postId}`, {
      params: { page, size },
    });
